...
- [ ] Create a **fine-grained PAT** scoped to `krech-et/kse-labs-deployment`
...
- [ ] Inputs:
  - `deployment-repo` (default: `krech-et/kse-labs-deployment`)
  - `service-name` (required -- used for commit messages AND to derive the deployment path)
  - `image-name` (e.g., `ghcr.io/krech-et/simple-go-service-a`)
  - `new-tag` (e.g., `v0.0.14`)
...
- [ ] Add `update-deployment` job to `simple-go-service-a/.github/workflows/release.yaml`:
  ...
  update-deployment:
    ...
    uses: krech-et/kse-labs-trusted-workflows/.github/workflows/update-deployment-manifest.yaml@main
    secrets:
      deployment-repo-token: ${{ secrets.DEPLOYMENT_REPO_TOKEN }}
    with:
      image-name: ghcr.io/krech-et/simple-go-service-a
...
image-name: ghcr.io/krech-et/simple-go-service-b
```
